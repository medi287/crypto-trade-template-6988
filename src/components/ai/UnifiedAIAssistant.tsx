...
interface AICapability {
  ...
  title: string;
  description: string;
  category: 'assistant' | 'prediction' | 'nlp' | 'analysis';
}
...
export function UnifiedAIAssistant() {
  const [activeTab, setActiveTab] = useState<'assistant' | 'prediction' | 'nlp' | 'analysis'>('assistant');
  const [searchQuery, setSearchQuery] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);

  // Capacités IA unifiées
  const aiCapabilities: Record<string, AICapability[]> = {
    ...
    prediction: [
      ...
      },
      {
        ...
        description: "Scoring automatique de conformité réglementaire et détection de risques juridiques",
        category: 'prediction'
      },
      {
      ...
      },
      {
        ...
        description: "Prévision des évolutions législatives et réglementaires",
        category: 'prediction'
      }
    ],
    nlp: [
        ...
        <CardHeader>
          <div className="text-center">
            ...
            <p className="text-gray-600 text-xl mb-6">
              Intelligence Artificielle avancée pour le droit algérien - Toutes les fonctionnalités IA en un seul endroit
            </p>
